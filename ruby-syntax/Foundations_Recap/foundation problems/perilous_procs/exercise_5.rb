...
# contains procs as both keys and values. The method should return a new sentence 
# where each word of the input sentence is changed by a value proc if the original 
# word returns true when passed into the key proc. If an original word returns true 
# for multiple key procs, then the value proc changes should be applied in the order 
# that they appear in the hash.

def procipher(sent, prcs)
  words = sent.split(' ')

  new_words = words.map do |word|
    new_word = word
    
    prcs.each do |k_prc, v_prc|
      if k_prc.call(word) == true
...
# true for multiple key procs, then only the value proc that appears earliest in the 
# hash should be applied.

def picky_procipher(sent, prcs)
  words = sent.split(' ')
